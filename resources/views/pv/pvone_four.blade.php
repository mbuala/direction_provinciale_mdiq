...
@section('content')
<form action="{{url('pvone_reser/'.$aviss->idss)}}" method="post">
    @csrf
            ...
            <div class="container">
                <div id="tap" class="elimin">
                    <div class="row hed">
                        ...
                        <div class="col">Eliminé</div>
                        <div class="col">Motif d'écartement</div>
                    </div>
                    <div class="cl">
                        @for ($i = 1; $i <= $aviss->concu; $i++)
                        <div class="row" >
                            <div class="col">{{$i}}</div>
                            <div class="col">{{$concurrents[$i-1]}} <input type="hidden" name='noms[]' value="{{$concurrents[$i-1]}}"></div>
                            <div class="col">
                                <input type="checkbox" name='el{{$i}}' value="Eliminer">
                            </div>
                            <div class="col">
                                <input type="text" class="form-control" placeholder="Motife" name="motif{{$i}}">
                            </div>
                        </div>
                        @endfor
                    </div>
